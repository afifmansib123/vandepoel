  ...
  try {
    ...
    if (success) {
      ...
      if (resultAsAny && resultAsAny.error) {
        if (error)
          toast.error(
            typeof resultAsAny.error.data?.message === "string"
              ? resultAsAny.error.data.message
              : error
          );
      } else {
    ...
    // This catch block is for when mutationFn (e.g., from .unwrap()) rejects
    const errorMessage =
      err?.data?.message ||
      err?.message ||
      error ||
...
interface AmplifyIdTokenPayload {
  // From idToken.payload
  sub: string;
  email?: string;
  phone_number?: string;
  name?: string;
...
interface AmplifyIdToken {
  // From session.tokens.idToken
...
export const createNewUserInDatabase = async (
  ...
  console.log(
    "[utils/createNewUserInDatabase] Called. UserID:",
    amplifyUser.userId,
    "Role:",
    userRole
  );
  ...
  console.log(
    `[utils/createNewUserInDatabase] Extracted for payload - CognitoID: "${cognitoIdForBody}", Name: "${name}", Email: "${email}", Phone: "${phoneNumber}"`
  );
  ...
  if (
    !userDataPayload.cognitoId ||
    !userDataPayload.name ||
    !userDataPayload.email
  ) {
    const missingFields: string[] = [];
    if (!userDataPayload.cognitoId)
      missingFields.push("cognitoId (from amplifyUser.userId)");
    if (!userDataPayload.name)
      missingFields.push("name/username (from Cognito attributes)");
    if (!userDataPayload.email)
      missingFields.push("email (from Cognito attributes)");

    const clientErrorMessage = `Client: Cannot create user profile in DB. Missing essential attributes from Cognito: ${missingFields.join(
      ", "
    )}. Please check Cognito User Pool attribute settings, app client read permissions, and ensure the user has these attributes set in Cognito.`;
    ...
    default:
      ...
      return {
        error: {
          ...
          data: {
            message: `Cannot create user: Unknown role '${userRole}'. Check Cognito custom:role attribute.`,
          },
        },
      };
  }
  ...
  console.log(
    `[utils/createNewUserInDatabase] Calling POST to ${endpointPath} with payload:`,
    userDataPayload
  );
  ...
  if (createUserResponse.error) {
    console.error(
      `[utils/createNewUserInDatabase] Backend error creating user (POST ${endpointPath}): Status ${createUserResponse.error.status}`,
      createUserResponse.error.data
    );
  } else {
    console.log(
      `[utils/createNewUserInDatabase] Successfully created user via POST to ${endpointPath}:`,
