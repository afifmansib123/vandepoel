...
import { FileText, CheckCircle, XCircle, Clock, Mail, Phone, User, ArrowRight, ArrowLeft } from "lucide-react";
...
const ManagerApplicationsPage = () => {
      ...
      const fetchAllApplications = async () => {
        ...
        try {
          ...
          if (!incomingRes.ok || !outgoingRes.ok) {
            throw new Error("Failed to fetch one or more application lists.");
          }
          ...
          // Simple de-duplication in case an application is somehow in both lists
          const uniqueApplications = Array.from(new Map(combinedApplications.map(app => [app._id, app])).values());
          
          setApplications(uniqueApplications.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
  ...
  const handleUpdateStatus = async (applicationId: string, newStatus: "approved" | "rejected") => {
    ...
    try {
      ...
      const originalApps = await fetch(`/api/applications?receiverId=${managerCognitoId}`).then(res => res.json()); // Simple refetch on error
  ...
  const renderContent = () => {
    ...
    if (applications.length === 0) return (
      <div className="text-center py-16 px-6 bg-white rounded-lg shadow-md">
        ...
        <p className="mt-1 text-gray-500">You have no incoming or outgoing applications.</p>
      </div>
    );
  ...
  return (
    <div className="dashboard-container bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8">
      <Header title="My Applications" subtitle="Review incoming requests and track your outgoing applications." />
...
const ApplicationCard = ({ application, currentUserId, onViewDetails, onUpdateStatus }: { 
    application: Application, 
    currentUserId: string,
    onViewDetails: () => void, 
    onUpdateStatus: (id: string, status: "approved" | "rejected") => void 
}) => {
    const { _id, propertyId, formData, status, createdAt, applicationType, receiverId } = application;
            ...
            <div className="p-5 flex flex-col md:flex-row gap-5 items-start">
                ...
                <div className="flex-grow">
                    <div className="flex justify-between items-start">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <span className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full ${isIncoming ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}`}>
                                    {isIncoming ? <ArrowLeft size={12} /> : <ArrowRight size={12} />}
                                    {isIncoming ? 'Incoming' : 'Outgoing'}
                                </span>
                                <span className="text-xs font-semibold text-blue-700 bg-blue-100 px-2 py-1 rounded-full inline-block">{formatApplicationType(applicationType)}</span>
                            </div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                        <div className="flex items-center gap-2"><User size={16} /><span>{formData.name}</span></div>
                        <div className="flex items-center gap-2"><Mail size={16} /><span>{formData.email}</span></div>
                        <div className="flex items-center gap-2"><Phone size={16} /><span>{formData.phone || 'N/A'}</span></div>
                    </div>
                    <p className="text-xs text-gray-400 mt-2">Received on: {new Date(createdAt).toLocaleDateString()}</p>
                </div>
...
const ApplicationDetailsModal = ({ application, currentUserId, onClose, onUpdateStatus }: { 
    application: Application, 
    currentUserId: string,
    onClose: () => void, 
    onUpdateStatus: (id: string, status: "approved" | "rejected") => void 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-6">
                    {/* UI for details remains the same */}
