...
type EditableTenantProfile = Partial<
  Omit<
    TenantProfile,
...
useEffect(() => {
    ...
    const fetchTenantProfile = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/tenants/${authUser.cognitoInfo.userId}`
        );
        ...
        const data: TenantProfile = await response.json();
        setTenantData(data);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const newProfileState: TenantProfile = {
        ...TenantData,
        ...formData,
  ...
  return (
    <div className="dashboard-container bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8">
      <Header
        title="My Profile"
        subtitle="Manage your Tenant profile and personal information"
      />
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-end -mt-16 mb-6">
              <div className="relative">
                <div className="w-32 h-32 rounded-full border-4 border-white shadow-lg overflow-hidden bg-gray-200 flex items-center justify-center">
                  {/* Default avatar for Tenants */}
                  <div className="w-full h-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center">
                    <span className="text-white text-4xl font-bold">
                      {TenantData.name?.charAt(0)?.toUpperCase() || "B"}
                    </span>
                  </div>
                </div>
              ...
              <div className="mt-4 sm:mt-0 sm:ml-6 flex-1">
                ...
                <p className="text-gray-500">{TenantData.email}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-900">
                {TenantData.favorites?.length || 0} properties saved
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-1">
              ...
              <p className="text-gray-900">
                {TenantData.properties?.length || 0} properties
              </p>
            </div>
          </div>
        </div>
