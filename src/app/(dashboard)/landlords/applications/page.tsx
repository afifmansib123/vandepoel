...
import { FileText, CheckCircle, XCircle, Clock, Mail, Phone, User } from "lucide-react";
...
const LandlordApplicationsPage = () => {
  ...
  useEffect(() => {
    if (landlordCognitoId) {
      const fetchApplications = async () => {
        ...
        setError(null);
        try {
          const response = await fetch(`/api/applications?receiverId=${landlordCognitoId}`);
    ...
    return (
      <div className="space-y-4">
        {filteredApplications.map(app => (
          <ApplicationCard key={app._id} application={app} onViewDetails={() => setSelectedApplication(app)} onUpdateStatus={handleUpdateStatus} />
        ))}
      </div>
    );
  };
  ...
  return (
    <div className="dashboard-container bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8">
      <Header title="Received Applications" subtitle="Review and manage applications for your properties." />
      ...
      {selectedApplication && (
        <ApplicationDetailsModal application={selectedApplication} onClose={() => setSelectedApplication(null)} onUpdateStatus={handleUpdateStatus} />
      )}
    </div>
  );
};
