...
export async function GET(request: NextRequest, context: HandlerContext) {
  ...
  console.log(
    `[API /landlords/:id GET] Handler invoked. Path param cognitoId: "${cognitoIdFromPath}"`
  );

  // Basic validation for cognitoId
  if (!cognitoIdFromPath || cognitoIdFromPath.trim() === "") {
    console.warn(
      "[API /landlords/:id GET] Invalid or missing cognitoId in path."
    );
    return NextResponse.json(
      {
        message:
          "Cognito ID path parameter is required and must be a non-empty string",
      },
      { status: 400 }
    );
  }
  ...
  await dbConnect();
  console.log(
    `[API /landlords/:id GET] DB connected. Querying for cognitoId: "${cognitoIdFromPath}"`
  );
  ...
  try {
    ...
    if (!landlord) {
      console.log(
        `[API /landlords/:id GET] MongoDB Query Result: Landlord with Cognito ID "${cognitoIdFromPath}" NOT FOUND.`
      );
      return NextResponse.json(
    ...
    console.log(
      `[API /landlords/:id GET] MongoDB Query Result: Found landlord "${
        landlord.name || "(name not set)"
      }" for Cognito ID "${cognitoIdFromPath}".`
    );
    ...
    console.error(
      `[API /landlords/:id GET] Database or other error fetching landlord "${cognitoIdFromPath}":`,
      error
    );
    const message =
      error instanceof Error ? error.message : "An unknown error occurred";
    return NextResponse.json(
      { message: `Error retrieving landlord: ${message}` },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: NextRequest, context: HandlerContext) {
  ...
  console.log(
    `[API /landlords/:id PUT] Handler invoked. Path param cognitoId: "${cognitoIdFromPath}"`
  );

  // Basic validation for cognitoId
  if (!cognitoIdFromPath || cognitoIdFromPath.trim() === "") {
    console.error(
      "Error: cognitoIdFromPath is invalid or missing from context.params in PUT request"
    );
    return NextResponse.json(
      { message: "Cognito ID is required in path and must be non-empty" },
      { status: 400 }
    );
  }
  ...
  await dbConnect();
  console.log(
    `[API /landlords/:id PUT] DB connected. Processing for cognitoId: ${cognitoIdFromPath}`
  );
  ...
  try {
    ...
    if (Object.keys(updateData).length === 1) {
      ...
      console.log(
        "[API /landlords/:id PUT] No valid fields provided for update."
      );
      return NextResponse.json(
    ...
    if (!updatedLandlord) {
      console.log(
        `[API /landlords/:id PUT] Landlord with Cognito ID "${cognitoIdFromPath}" not found for update or no changes made.`
      );
      return NextResponse.json(
        { message: "Landlord not found or no changes made" },
        { status: 404 }
      );
    }

    console.log(
      `[API /landlords/:id PUT] Updated landlord name: ${
        updatedLandlord.name || "(name not set)"
      }`
    );
    ...
    const responseLandlord: LandlordResponse = {
      ...updatedLandlord,
      _id:
        typeof updatedLandlord._id === "string"
          ? updatedLandlord._id
          : updatedLandlord._id.toString(),
    ...
    console.error(
      `[API /landlords/:id PUT] Error updating landlord "${cognitoIdFromPath}":`,
      error
    );
    ...
    const message =
      error instanceof Error ? error.message : "An unknown error occurred";
    return NextResponse.json(
      { message: `Error updating landlord: ${message}` },
      { status: 500 }
    );
  }
}
