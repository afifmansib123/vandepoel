  ...
  try {
    ...
    if (!cognitoId || !name || !email) {
      return NextResponse.json(
        {
          message:
            "Missing required fields: cognitoId, name, email, phoneNumber",
        },
        { status: 400 }
      );
    }
    ...
    if (existingManager) {
      return NextResponse.json(
        { message: "Manager with this Cognito ID already exists" },
        { status: 409 }
      );
    }
    ...
    const lastManager = (await Manager.findOne()
      .sort({ id: -1 })
      .select("id")
      .lean()) as ManagerData | null;
    const nextManagerId =
      (lastManager && typeof lastManager.id === "number" ? lastManager.id : 0) +
      1;
    ...
    if (error.code === 11000) {
      return NextResponse.json(
        {
          message:
            "Duplicate key error. A manager with this identifier might already exist.",
        },
        { status: 409 }
      );
    }
