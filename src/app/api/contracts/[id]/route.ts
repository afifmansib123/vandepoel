...
export async function GET(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const contract = await Contract.findById(params.id).populate('propertyId', 'name');
    if (!contract) {
...
export async function PUT(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const updatedContract = await Contract.findByIdAndUpdate(params.id, updateData, { new: true, runValidators: true });
    if (!updatedContract) {
      return NextResponse.json({ message: 'Contract not found' }, { status: 404 });
    }
