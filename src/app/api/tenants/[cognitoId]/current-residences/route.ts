...
export async function GET(
  ...
  try {
    // First, find the tenant's MongoDB _id using their cognitoId
    const tenant = await Tenant.findOne({ cognitoId }).select('_id').lean().exec();
    if (!tenant) {
    ...
    const activeLeases = await Lease.find({
      tenant: tenantObjectId, // Assuming Lease.tenant stores Tenant's MongoDB _id
      startDate: { $lte: now },
      endDate: { $gt: now }
    ...
    console.error(`Error retrieving current residences for tenant ${cognitoId}:`, error);
